...
function MetricCard({ title, value, subtext, tone }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 34, lineHeight: 1.05, fontWeight: 700, color: "#0F172A", letterSpacing: -0.8 }}>{value}</div>
...
export default function Page() {
  ...
  const metrics = useMemo(() => {
    ...
    const predictability = Math.round(clamp(100 - (top3 * 0.3 + founder * 0.3 + (100 - forecast) * 0.4), 0, 100));
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#F8FAFC", padding: 24, fontFamily: "Arial, sans-serif", color: "#0F172A" }}>
      <div style={{ maxWidth: 1220, margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <h1 style={{ margin: 0, fontSize: 40, lineHeight: 1.05, letterSpacing: -1.2 }}>Revenue Fragility Assessment</h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(320px, 400px) minmax(0, 1fr)", gap: 24, marginTop: 24 }}>
          <div style={{ background: "white", border: "1px solid #E2E8F0", borderRadius: 28, padding: 24, boxShadow: "0 1px 2px rgba(15, 23, 42, 0.04)" }}>
            ...
            <form onSubmit={submitLead} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <TextField label="Lead capture email" value={email} setValue={setEmail} placeholder="name@company.com" />
              ...
              {submitted ? (
                <div style={{ borderRadius: 16, border: "1px solid #A7F3D0", background: "#ECFDF5", padding: 14, fontSize: 13, lineHeight: 1.6, color: "#047857" }}>
                  Lead captured locally. Next step is wiring this button to Airtable or HubSpot.
                </div>
              ) : null}
            </form>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            <div style={{ background: "white", border: "1px solid #E2E8F0", borderRadius: 28, padding: 20, boxShadow: "0 1px 2px rgba(15, 23, 42, 0.04)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
              <div>
                <div style={{ fontSize: 13, color: "#64748B", fontWeight: 600 }}>Assessment for</div>
                <div style={{ marginTop: 6, fontSize: 30, fontWeight: 700, letterSpacing: -0.8 }}>{companyName || "Untitled Company"}</div>
              </div>
              <div style={{ border: `1px solid ${risk.border}`, background: risk.bg, color: risk.text, borderRadius: 999, padding: "8px 12px", fontWeight: 700, fontSize: 13 }}>
                {risk.label}
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 16 }}>
              ...
              <MetricCard
                ...
                tone={metrics.predictability < 50 ? { bg: "#FEF2F2", border: "#FECACA", text: "#B91C1C" } : metrics.predictability < 70 ? { bg: "#FFFBEB", border: "#FDE68A", text: "#B45309" } : { bg: "#ECFDF5", border: "#A7F3D0", text: "#047857" }}
              />
            </div>

            <div style={{ background: "white", border: "1px solid #E2E8F0", borderRadius: 28, padding: 24, boxShadow: "0 1px 2px rgba(15, 23, 42, 0.04)" }}>
              <div style={{ fontSize: 24, fontWeight: 700, marginBottom: 6 }}>Driver Analysis</div>
              <div style={{ fontSize: 14, lineHeight: 1.6, color: "#64748B", marginBottom: 18 }}>Where structural exposure is being created.</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 16 }}>
                <DriverCard
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 16 }}>
              <div style={{ background: "white", border: "1px solid #E2E8F0", borderRadius: 28, padding: 24, boxShadow: "0 1px 2px rgba(15, 23, 42, 0.04)" }}>
                ...
                <div style={{ fontSize: 14, lineHeight: 1.8, color: "#475569" }}>
                  <p style={{ marginTop: 0 }}>
                    This analysis indicates structural exposure driven by the interaction of revenue concentration, founder-dependent ownership, and forecast variability.
                  </p>
                  <p>
                    As the business scales, this reduces predictability and introduces risk to both revenue stability and margin integrity. The issue is less about visibility and more about whether the operating model translates that visibility into manager-level control.
                  </p>
                </div>
                <div style={{ marginTop: 14, border: "1px solid #E2E8F0", background: "#F8FAFC", borderRadius: 20, padding: 16 }}>
                  <div style={{ fontWeight: 700, marginBottom: 6 }}>Recommended next step</div>
                  <div style={{ fontSize: 14, lineHeight: 1.7, color: "#475569" }}>
                    Validate where this exposure is originating structurally across account ownership, forecast construction, and margin drivers, then determine whether the exposure is large enough to justify a deeper diagnostic.
                  </div>
                </div>
              </div>
              ...
              <div style={{ background: "white", border: "1px solid #E2E8F0", borderRadius: 28, padding: 24, boxShadow: "0 1px 2px rgba(15, 23, 42, 0.04)" }}>
                ...
                <div style={{ fontSize: 14, lineHeight: 1.6, color: "#64748B", marginBottom: 18 }}>Position against a stronger operating profile.</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                  <ProgressRow label="Concentration" current={`${top3}%`} target="20–30%" progress={100 - Math.max(0, top3 - 20) * 4} />
                  <ProgressRow label="Founder Dependency" current={`${founder}%`} target="<25%" progress={100 - Math.max(0, founder - 15) * 2.5} />
                  <ProgressRow label="Forecast Accuracy" current={`${forecast}%`} target="85–95%" progress={(forecast / 95) * 100} />
                  <ProgressRow label="Margin" current={`${margin}%`} target={`${targetMargin}%+`} progress={(margin / Math.max(targetMargin, 1)) * 100} />
                </div>
              </div>
            </div>
          </div>
